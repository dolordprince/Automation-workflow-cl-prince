...
// ✅ CORRECT OLLAMA IMPORT - no destructuring, no `new`
const ollama = require('ollama');

// Optional: HuggingFace (skip if DNS issues)
...
async function callOllama(prompt) {
  // ✅ CORRECT API CALL for ollama npm package
...
function generateStaticFallback(topic) {
  return `<!DOCTYPE html>
<html><head><title>${topic}</title><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<style>body{font-family:system-ui,sans-serif;background:linear-gradient(135deg,#0f0f1a,#1a1a2e);color:#e6e6ff;margin:0;padding:2rem}
.container{max-width:1200px;margin:0 auto}.hero{text-align:center;padding:4rem 2rem}
...
.dashboard{background:#1e1e3f;border-radius:1rem;padding:2rem;margin-top:2rem;box-shadow:0 10px 40px rgba(0,0,0,.3)}</style></head><body><div class="container"><div class="hero"><h1>✨ ${topic}</h1><p>AI-generated premium template • Fallback mode</p></div>
<div class="dashboard"><h3>📊 Dashboard Preview</h3><p>Content generated with static fallback. Configure Ollama/HF for dynamic AI.</p></div></div></body></html>`;
}
...
async function main() {
  ...
  const prompt = `Generate a single-file HTML landing page for: "${topic}". 
Include: modern dark luxury design, gradient accents, responsive layout, 
dashboard preview section, CTA buttons, and inline CSS. No external dependencies. 
Output ONLY valid HTML, no markdown, no explanations.`;
  ...
  // Optional: auto-push if enabled
  if (process.env.AUTO_PUSH === 'true' && process.env.GITHUB_TOKEN) {
    try {      console.log('📤 PUSHING TO GITHUB...');
